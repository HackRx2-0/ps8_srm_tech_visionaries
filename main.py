...
class FaceDetection:
    ...
    def emotion_detector(self):
        ...
        for (x, y, w, h) in faces:
            ...
            self.response= self.response + "\nEmotion: "+ labels[int(np.argmax(yhat))]                                             ##CHECK
...
def driver(img_file):
    ...
    face_det.main()
    op = face_det.return_string()
    return img,op

input_type = gr.inputs.Image(type="file")
output_type = [gr.outputs.Image(), gr.outputs.JSON()]


iface = gr.Interface(fn=driver, inputs=input_type, outputs=output_type, live=True)
